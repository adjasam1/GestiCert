...
//	@GetMapping
//	@PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_SERVICE') or hasRole('ROLE_DEV')")
//	public List<AppUserDto> getAllUsersWithAuth() {
//		return userServ.findAllUsers().stream().map(user -> new AppUserDto(user.getIdRHUser(), user.getRoleList())).collect(Collectors.toList());
//	}
//	@GetMapping("/idrh={idRHUser}")
//	@PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_SERVICE') or hasRole('ROLE_DEV')")
//	public ResponseEntity<AppUserDto> getUserByIdRH(@PathVariable String idRHUser) {
//		Optional<User> user = userServ.findUserByIdRHUser(idRHUser);
//		if (user.isPresent()) {
//			return ResponseEntity.ok(new AppUserDto(user.get().getIdRHUser(), user.get().getRoleList()));
//		} else {
